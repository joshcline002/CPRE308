...
void *workThread(void * args){
    ...
    if(strcmp(commandToRun[0],"CHECK")==0){
        ...
        if(atoi(commandToRun[1])<=accounts){
            ...
            //Check that the bank can be locked.
            while( !(pthread_mutex_lock(&(workLocks))==0)){
                usleep(500);
            }
            ...
            //Unlock the bank.
            pthread_mutex_unlock(&(workLocks));
	...
	//Checks to see if the bank can be locked.
	while( !(pthread_mutex_lock(&(workLocks))==0)){
                usleep(500);
            }    
        int done = 0;
        ...
        //Locks file to write to and writes then unlocks
        pthread_mutex_lock(&(fileLock));
        output = fopen(file,"a");
        gettimeofday(&finishTime,NULL);
